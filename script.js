...
///message.textContent='we use cookiedfor improved functinalites and analaysis'
message.innerHTML =
  'we use cookied for improved functinalites and analaysis, <button class="btn btn--close-cookie">Got it!</button>';
...
// h2.closest('h1').style.background = 'var(--gradient-primary)';
...
tabscontainer.addEventListener('click', function (e) {
  ...
  // clicked.classList.add('opeartions__tab--active');
...
const initalcard = section1.getBoundingClientRect();
//console.log(initalcard);
window.addEventListener('scroll', function (e) {
  // console.log(window.scrollY);
  if (this.window.scrollY > initalcard.top) nav.classList.add('sticky');
...
const slider = function () {
  ...
  const docContainer = document.querySelector('.dots');
  let curSlide = 0;
  const maxslide = slides.length;
  //function
  const createDots = function () {
    slides.forEach(function (_, i) {
      docContainer.insertAdjacentHTML(
        'beforeend',
